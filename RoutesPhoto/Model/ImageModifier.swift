...
class PinchZoomView: UIView {
    ...
    @objc private func pinch(gesture: UIPinchGestureRecognizer) {
        ...
        case .changed:
            let newScale = min(max(lastScale*gesture.scale, minScale), maxScale)
            scaleChange(newScale)
        case .ended:
            let newScale = min(max(lastScale*gesture.scale, minScale), maxScale)
            lastScale = min(max(newScale, minScale), maxScale)
            scaleChange(lastScale)
